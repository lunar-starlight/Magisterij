...
TODO: morfizme je treba nekje not uštulit…
\begin{trditev}
...
% \begin{definicija}
%   Za vsako preslikavo \(R : B → ℒ\) in morfizem \(f : A ↬ B\), lahko definiramo
%   \(R(f(a)) ≔ \exist{b : B}{b = f(a) ∧ R(b)}\).

%   V posebnem je potem \(f(a) = g(a)\) ekvivalenten
%   \(\exist{b : B}{b = f(a) ∧ b = g(a)}\)
% \end{definicija}
\begin{lema}
...
\begin{dokaz}
  ...
  % Pokazati moramo, da je ta implikacija
  % ekvivalenca, tako da naj bo \(b = g(a)\) in dokazujemo \(b = f(a)\).
  % Vrednost \(g(a) = f(a)\) je \(\exist{b' : B}{b' = g(a) ∧ b' = f(a)}\),
  % in po predpostavki je enaka \(b = f(a)\), tako da moramo dokazati, da tak
  % \(b'\) obstaja.
  % Vemo, da velja \(b = g(a) ⇒ \e a\) in \(\e a ⇔ \exist{b : B}{b = f(a)}\).
  % Prav tako po predpostavki velja \(b = f(a) ⇒ b = f(a) ∧ b = g(a)\). Tako
  % lahko tvorimo verigo implikacij
  % \begin{align*}
  %   b = g(a)
  %   &⇒ b = g(a) ∧ \exist{b' : B}{b' = g(a) ∧ b' = f(a)}\\
  %   &⇒ \exist{b' : B}{b = g(a) ∧ b' = g(a) ∧ b' = f(a)}\\
  %   &⇒ \exist{b' : B}{b = b' ∧ b' = f(a)}\\
  %   &⇒ b = f(a)
  % \end{align*}
...
NOTE: A je to važno? no, sej to na začetku obljubim, ampak lahko tudi brez
\begin{lema}
