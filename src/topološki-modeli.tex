...
\begin{primer}
  Naj bo \(P ⊑ A\). Tvorimo \(ℒ\)-množico \(A/P\) z nosilcem \(A\) in enakostjo
  \(\i{a = a'} ≔ \i{a =_A a'}∧P(a)\). Zaradi preglednosti posebej označimo
  le enakost na \(A\).

  To je simetrično, saj je \(P\) ekstenzionalen. Tranzitivnost je malo bolj
  zanimiva. Očitno iz \(\i{a = a'}∧\i{a' = a''}\) sledi
  \(\i{a=_A a''}∧P(a)∧P(a')\) in, ker smo imeli \(\i{a = a'}∧P(a')\), lahko prej
  tisti \(P(a')\) spremenimo v \(P(a)\), tako da sledi \(\i{a =_A a''}∧P(a)\).

  Poleg tega je identitetna relacija med \(A/P\) in \(A\), torej preslikava
  \(\p{a, a'} ↦ \i{a =_A a'}\), monomorfizem. To sledi iz dejstva, da če je
  \(f : B ↬ A/P\), je \(\i{a \sim \id∘f(b)} = \i{a \sim f(b)}\).
\end{primer}
...
\begin{proof}
  ...
  \(A/P\). Ta se v \(A\) slika z identiteto, ki je po gornjem monomorfizem.
  ...
  Obratno, če je \(P ⊑ A\) je \(a ∈ \id[A/P]\) natanko tedaj, ko velja \(a ∈ P\).
\end{proof}
...
\begin{konstrukcija}
  Za vsak \(a ∈ A\) lahko tvorimo enojec \(\s a ≔ \set{x : A}{x = a}\).
\end{konstrukcija}
\begin{dokaz}
...
Ker na vrednost \(f(a)\) želimo misliti kot term tipa \(B\), bi bilo priročno,
če lakho kako definiramo, kako se \(f(a)\) uporabi v formulah.
\begin{definicija}
...
\begin{dokaz}
  ...
  % TODO: refactor? tranzitivnost ~ nekje? also, nek ap in cong za ~?
  \(g(b) \sim g(f(a)) = h(f(a)) \sim h(b)\).

  Po drugi strani pa predpostavimo, da je \(f\) epimorfizem.
  % TODO: dokaz

  NOTE: Borceux naredi nek kompliciran argument, a obstaja kaj boljšega v
  internem jeziku?
