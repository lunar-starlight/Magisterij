...
Potem je pa za \(U ≔ \{1\}\) \(¬U = ∅\), torej \(U ∨ ¬U = U ≠ X\). Torej logika
tega prostora ni klasična, in tam gornja enakost ne velja.

% Konjunkcijo \(U ∧ V\) ``resnično na \(U\) in \(V\)'', torej je natanko \(U ∩ V\).
% Podobno je disjunkcija \(U ∨ V\) potem \(U ∪ V\). Pri negaciji moramo malo
% razmisliti. Naivno bi za ``negacijo množice'' vzeli komplement, a ta ni nujno
% odprt, zato vzamemo njegovo notranjost. Temu se potem reče tudi zunanjost
% množice, označeno \(¬U = \ext{U} = \int{\p{Uᶜ}}\). Očitno sta \(⊤\) in \(⊥\)
% natanko \(X\) in \(∅\). Ostane še implikacija.


% Iz konstruktivne matematike vemo,
% da je resničnostna vrednost \(U ⇒ V\) je natanko tista, za katero velja, da je
% \(W ⊆ U ⇒ V\) natanko tedaj ko \(W ∩ U ⊆ V\). To pa pomeni, da je
% \(U ⇒ V ≔ ⋃\set{W ⊆ X}{W ∩ U ⊆ V}\).
% To je malo v nasprotju s klasično definicijo, kjer se pogosto reče, da je
% \(U ⇒ V = V ∨ ¬U\). Ampak to se ne bo lepo obnašalo že za zelo preproste
% prostore, na primer prostor Sierpinskijeva. Pričakovali bi, da \(U ⇒ U\) vedno
% velja, torej da je \(U ⇒ U = X\). A vendar, če bi veljala tista klasična
% enakost, bi za \(U = \{1\}\) dobili \(\{1\} ∨ ¬\{1\}\). Ampak \(¬\{1\} = ∅\),
% torej bi bilo \(\{1\} ⇒ \{1\} = \{1\}\), kar pa ni zaželeno.

% Lahko pa vseeno rečemo, da je \(U ⇒ V = \int{\p{V ∪ Uᶜ}}\). Res,
% \(W ∩ U ⊆ V\) natnako tedaj ko velja \(W ⊆ V ∪ Uᶜ\), torej je tista unija zgoraj
% natanko notranjost \(V ∪ Uᶜ\).
Torej imamo sledeči slovar:
...
Naj bo \(P : 𝒪X → 𝒪X\). Potem se kvantifikatorji interpretirajo kot neskončne
unije in preseki. Seveda pa topologija ni zaprta za poljubne preseke, tako da
moramo tam vzeti notranjost preseka.
\begin{align*}
  \exist{p : Ω}{P(p)} &≔ ⋃_{U ∈ 𝒪X}P(U)\\
  \for{p : Ω}{P(p)}   &≔ \int{\p{⋂_{U ∈ 𝒪X}P(U)}}
\end{align*}
Zares bi lahko rekli tudi malo več, če je \(A\) poljubna množica in \(P : A → 𝒪X\) 
lahko definiramo \(\exist{a : A}{P(a)}\) kot \(⋃_{a ∈ A}P(a)\). Tu v bistvu
množico v našo logiko vložimo kot diskreten prostor, kar dela dobro za na primer
množico naravnih števil, se pa izkaže, da za recimo realna števila ne deluje.
Res, realna števila (v večini logik) ne bodo diskreten prostor! Si bomo pa
vseeno podrobneje pogledali kdaj točno so.
...
aksiomoma simetričnosti in tranzitivnosti delno urejenih množic, tako da bi
lahko rekli, da so \(ℒ\)-množice natanko množice opremljene z (interno) delno
...
\begin{primer}
  ...
  je \(ℒ\)-množica. To bomo označevali z \(Δℕ\).
\end{primer}
