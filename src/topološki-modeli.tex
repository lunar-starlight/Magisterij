...
\end{definicija}
\begin{opomba}
  Na relacijo \(b ~ f(a)\) zares želimo gledati kot enaksot, ampak ne nujno
...
TODO: uvest kaj so terms, \(α : A\), etc

\begin{table}[h]
  \centering
  \begin{tabularx}{0.6\textwidth}{p{0.2\textwidth} X}
    \(\i{a}\)    & \(≔ a\), za \(a ∈ A\)\\
    \(\i{f(α)}\) & \(≔ f{\p{\i α}}\), kjer je \(f : A → B\) in \(α : A\)
  \end{tabularx}

  \caption{Interpretacija termov v internem jeziku}
  \label{tab:int-term}
\end{table}
Vrednosti \(f(a)\) še ne moramo podati neposredne interpretacije, saj kot
rečeno njihova vrednost ne obstaja, lahko pa bomo definirali kaj pomeni
...
\begin{definicija}
  ...
  Potem lahko \(R(f(a))\) interpretiramo kot \(\exist{b : B}{b ~ f(a)∧R(b)}\).
\end{definicija}
\begin{opomba}
...
\begin{proof}
  ...
  enakost razpišemo, dobimo da obstaja \(b' ∈ B\), tako da je \(b' ~ f(a)\) in
  \(b' ~ g(a)\). Potem je pa \(b = b'\), torej je \(b = b' ~ g(a)\). Ker je
  vloga \(f\) in \(g\) simetrična, ter \(a\) in \(b\) poljubna, velja
  \(b ~ f(a)\) natanko tedaj, ko velja \(b ~ g(a)\), torej sta \(f\) in \(g\)
  enaka.
\end{proof}
\begin{opomba}
  Ker je \(f(a) = g(a)\) ekvivalentno \(f(a) ~ g(a)\) zadošča pokazati zgolj
  \[ ⊢ \for{a : A}{f(a) = g(a)}\text. \]
\end{opomba}
...
\begin{proof}
  ...
  \(f(g(*)) ~ f(a) = f(a') ~ f(h(*))\), torej je \(f∘g = f∘h\). Od tod pa sledi,
  da je \(g = h\), torej velja \(a ~ g(*) = h(*) ~ a'\).
\end{proof}
