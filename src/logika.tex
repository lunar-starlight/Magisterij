...
\begin{dokaz}
  ...
  Princip končne izbire pravi, da če je \(A\) končna, torej izomorfna neki
  standardni končni množici \(\{1,…,n\}\), in je \(R : A×B → Ω\) celovita, tedaj
  obstaja funkcija izbire \(f : A → B\), da je \(\for{a:A}{R(a,f(a))}\).

  Brez škode za splošnost lahko torej predpostavimo, da je \(A = \{1,…,n\}\).
  Če sedaj razpišemo, kaj gornje pomeni, dobimo, da če obstajajo \(bₖ:B\), da velja
  \(R(1,b₁)∨…∨R(n,bₙ)\), obstaja končno zaporedje (torej, obstajajo \(bₖ:B\)),
  da velja \(R(1,b₁)∨…∨R(n,bₙ)\)… To sta pa isti stvari, tako da je to preprosto res.
\end{dokaz}
...
\begin{definicija}
  ...
  Navadna \emph{kripkejeva shema} je kripkejeva shema za \(Σ₀¹\), torej za
  množico \(\set{α \apart 0}{α : 2^ℕ} ⊆ Ω\).

  \emph{Analitična kripkejeva shema} je kripkejeva shema za \(Σ_ℝ\), torej za
  množico \(\set{x > 0}{x : ℝ} ⊆ Ω\).
\end{definicija}
